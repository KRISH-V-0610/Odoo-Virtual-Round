...
const ProductCard = ({ title, price, index }) => (
    <article className="rounded-2xl border border-slate-200 bg-white shadow-sm hover:shadow-md transition-all overflow-hidden group">
        ...
        <div className="p-3">
            ...
            <Link
                to="/product"
                className="mt-2 w-full py-1.5 text-xs bg-slate-100 hover:bg-emerald-100 text-slate-700 rounded-lg transition-colors flex items-center justify-center gap-1"
             
                onClick={(e) => e.stopPropagation()} // if the whole card is clickable, keep clicks scoped
            >
